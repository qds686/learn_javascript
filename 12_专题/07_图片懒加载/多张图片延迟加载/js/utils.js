(function () {
  ...
  */
  ...
  // => DOM相关方法封装
  // 绑定事件
  const addEvent = function addEvent(el, type, fn) {
  ...
  const removeEvent = function removeEvent(elem, type, fn) {
  ...
  // 阻止事件冒泡
  const cancelBubble = function cancelBubble(e) {
  ...
  const preventDefaultEvent = function preventDefaultEvent(e) {
    var e = e || window.event;
    if (e.preventDefault) {
  ...
  const pagePos = function pagePos(e) {
  ...
  // 获取滚动条的滚动距离
  const getScrollOffset = function getScrollOffset() {
   ...
   */
  const getStyles = function getStyles(elem, prop) {
  ...
  // 获取浏览器真实宽高
  const getScrollSize = function getScrollSize() {
  ...
  // 查看浏览器窗口可视区域的宽高
  const getViewportSize = function getViewportSize() {
  ...
  const getElemDocPosition = function getElemDocPosition(el) {
  ...
  const elemChildren = function elemChildren(node) {
    var temp = {
      'length': 0,
  ...
  // 获取节点的第N个父元素
  const elemParent = function elemParent(node, n) {
    var type = typeof (n);
   ...
   */
  const setTplToHTML = function setTplToHTML(tpl, regTpl, opt) {
